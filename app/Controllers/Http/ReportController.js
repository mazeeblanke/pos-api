...
const { parseDateTime, ucFirst } = require('../../../utils/helper')
...
class ReportController {
  async index ({ request, response }) {
    ...
    if (!['asc', 'desc'].includes(direction)) {
      messages.push('Direction can only be -asc or -desc')
    }

    if (!filters.includes(type)) {
      messages.push(`Filter ${type} does not exist for ${report_type}s!`)
    }
    ...
    Results = await this[`fetch${ucFirst(report_type)}Report`] (payload)
  ...
  async productreport ({ request, response }) {
    const reqData = request.all()
    const from_time = reqData.from_time
      ? parseDateTime(reqData.from_time)
      : parseDateTime('0001-01-01')
    const to_time = reqData.to_time
      ? parseDateTime(reqData.to_time)
